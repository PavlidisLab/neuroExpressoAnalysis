...
# detect biological gender
bioGen = bioGender(humanExp) %>% replaceElement(c('F' = 'Female','M' = 'Male')) %$% newVector
...
write.table(unique(c(outliers,badSexSamples)),
            'data-raw/TrabzuniRegions/outliers', 
...
medExp = exprData %>%
    unlist %>% median
# we save this to remove genes in subsets of the dataset
TrabzuniMedExp = medExp
use_data(TrabzuniMedExp,overwrite = TRUE)
rowMedians = exprData %>% 
    apply(1, function(x){
